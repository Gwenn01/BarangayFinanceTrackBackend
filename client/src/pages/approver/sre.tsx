...
export default function ApproverSRE() {
  ...
  const approveMutation = useMutation({
    ...
    onError: (error: Error) => {
      toast({
        ...
        description: error.message || "Failed to approve transaction. Please try again.",
      });
    },
  });
  ...
  const flagMutation = useMutation({
    mutationFn: async ({
      ...
      const payload =
        type === "collection"
          ? { collection_id: id, reviewed_by: parseInt(reviewedBy), comment, flag_type: "collection" }
          : { disbursement_id: id, reviewed_by: parseInt(reviewedBy), comment, flag_type: "disbursement" };
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="review-comment" className="text-sm">
                  Comment{" "}
                  {reviewAction === "flagged" && <span className="text-red-600">*</span>}
                </Label>
                <Textarea
                  id="review-comment"
                  placeholder={
