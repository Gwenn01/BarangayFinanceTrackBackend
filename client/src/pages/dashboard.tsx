...
import type { FinancialSummary, MonthlyData, CategoryData } from "../../../shared/schema";
...
export default function Dashboard() {
  ...
  const { data: revenueByCategory, isLoading: revenueCategoryLoading } = useQuery<CategoryData[]>({
    queryKey: ["/api/revenue-by-category"],
  });

  const { data: expenseByCategory, isLoading: expenseCategoryLoading } = useQuery<CategoryData[]>({
    queryKey: ["/api/expense-by-category"],
  });

  if (summaryLoading || monthlyLoading || revenueCategoryLoading || expenseCategoryLoading) {
    return (
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="font-poppins">Revenue vs Expenses - Monthly Comparison</CardTitle>
        </CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={monthlyData || []}>
              ...
              />
              <Legend />
              <Bar dataKey="revenues" fill="hsl(var(--chart-1))" name="Revenues" radius={[4, 4, 0, 0]} />
