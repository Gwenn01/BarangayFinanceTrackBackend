...
import { type BudgetEntry, type InsertBudgetEntry } from "../../../../shared/schema";
  ...
  const createMutation = useMutation({
    ...
    onError: (error: Error) => {
      toast({
        ...
        description: error.message || "Failed to add budget entry. Please try again.",
      });
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error: Error) => {
      toast({
        ...
        description: error.message || "Failed to update budget entry. Please try again.",
      });
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: Error) => {
      toast({
        ...
        description: error.message || "Failed to delete budget entry. Please try again.",
      });
    },
  });
    ...
    <EncoderLayout>
      <div className="p-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-foreground font-poppins">Annual Budget Ordinance (ABO)</h1>
            <p className="text-muted-foreground mt-1">Manage annual budget allocations and appropriations</p>
          </div>
          <Button className="gap-2" onClick={handleCreate} data-testid="button-add-entry">
      ...
      <Card className="bg-gradient-to-br from-chart-1/5 to-chart-1/10 shadow-lg">
        <CardHeader>
          <CardTitle className="text-base font-medium text-muted-foreground">Total Budget Allocation</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-3xl font-bold text-chart-1" data-testid="text-total-allocated">
      ...
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="font-poppins">Budget Entries - {new Date().getFullYear()}</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="border rounded-lg overflow-hidden">
              <Table>
                ...
                <TableBody>
                  {entries.map((entry) => (
                    <TableRow key={entry.id} data-testid={`row-entry-${entry.id}`}>
                      ...
                      <TableCell>{format(new Date(entry.transactionDate), "MMM dd, yyyy")}</TableCell>
                ...
                <TableFooter>
                  <TableRow>
                    <TableCell colSpan={5} className="text-right font-semibold">Total Budget Allocation:</TableCell>
                    <TableCell className="text-right font-bold">{formatCurrency(totalAllocated.toString())}</TableCell>
                    <TableCell></TableCell>
                  </TableRow>
                </TableFooter>
              </Table>
