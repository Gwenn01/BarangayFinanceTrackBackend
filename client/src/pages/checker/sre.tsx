...
export default function CheckerSRE() {
  ...
  const reviewMutation = useMutation({
    mutationFn: async ({ id, type, comment }: { id: string; type: "collection" | "disbursement"; comment: string }) => {
      const endpoint = type === "collection" ? `/api/collections/${id}/review` : `/api/disbursements/${id}/review`;
      return apiRequest("PATCH", endpoint, { status: "flagged", comment, reviewedBy: "Checker" });
    },
  ...
  const handleReviewClick = (transaction: Collection | Disbursement, type: "collection" | "disbursement") => {
    ...
    <CheckerLayout>
      <div className="p-8 space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Flag transactions with errors for review</p>
        </div>
